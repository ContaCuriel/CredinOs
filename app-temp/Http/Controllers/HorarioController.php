...
class ListaDeRayaSheetExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithTitle, WithEvents, WithColumnFormatting
{
    ...
    private function calculateResults(): void
    {
        ...
        $empleados = Empleado::where('status', 'Alta')
            ...
            ->with(['puesto', 'horario']) // Se carga la relación con el horario
            ->get();
        ...
        foreach ($empleados as $empleado) {
            ...
            $diasFalta = $asistencias->where('status_asistencia', 'Falta')->count();
    
            // Se filtran solo las asistencias que no son faltas para calcular los retardos.
            $asistenciasPresentes = $asistencias->where('status_asistencia', '!=', 'Falta');
    
            // Se verifica que el empleado tenga un horario asignado.
            if ($empleado->horario) {
                $horario = $empleado->horario;
                // Mapeo de nombres de días de la semana de inglés (de Carbon) a español (de la BD).
                $mapaDias = [
                    'Monday'    => 'lunes',
                    'Tuesday'   => 'martes',
                    'Wednesday' => 'miercoles',
                ...
                foreach ($asistenciasPresentes as $asistencia) {
                    // Se asume que la columna 'fecha' de la tabla 'asistencias' es de tipo DATETIME.
                    $fechaHoraLlegada = Carbon::parse($asistencia->fecha);
                    $nombreDiaIngles = $fechaHoraLlegada->format('l'); // 'Monday', 'Tuesday', etc.
                    $nombreDiaEspanol = $mapaDias[$nombreDiaIngles] ?? null;
    
                    if ($nombreDiaEspanol) {
                        $campoDiaEntrada = $nombreDiaEspanol . '_entrada'; // Ej: 'lunes_entrada'
                        $horaEntradaProgramadaStr = $horario->$campoDiaEntrada;
    
                        // Se procede solo si hay una hora de entrada programada para ese día.
                        if ($horaEntradaProgramadaStr) {
                            // Se crea un objeto Carbon para la hora de entrada programada en la fecha de la asistencia.
                            $horaEntradaProgramada = Carbon::createFromTimeString($horaEntradaProgramadaStr);
                            $fechaHoraEntradaProgramada = $fechaHoraLlegada->copy()->setTimeFrom($horaEntradaProgramada);
    
                            // Se compara la hora de llegada real con la hora de entrada programada.
                            // Un retardo se cuenta si la hora de llegada es estrictamente posterior a la programada.
                            if ($fechaHoraLlegada->gt($fechaHoraEntradaProgramada)) {
                                $diasRetardo++;
                            }
                        }
                    }
    ...
    public function headings(): array
    {
        return [
            ...
            'R', 'F', // Columnas para Retardos y Faltas

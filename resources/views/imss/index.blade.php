                        ...
                        <tbody>
                            @forelse ($empleados as $empleado)
                                <tr>
                                    ...
                                    <td>
                                        {{ $empleado->patronImss ? $empleado->patronImss->nombre_comercial : 'N/A' }}
                                        @if($empleado->estado_imss == 'Alta' && $empleado->sdi)
                                            <br><small class="text-muted fw-bold">SDI: ${{ number_format($empleado->sdi, 2) }}</small>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        @if (!$empleado->estado_imss || $empleado->estado_imss != 'Alta')
                                            <button type="button" class="btn btn-sm btn-success btn-registrar-alta-imss"
                                                ...
                                                title="Registrar Alta IMSS">
                                                <i class="bi bi-shield-plus"></i> Alta
                                            </button>
                                        @endif
                                        
                                        @if ($empleado->estado_imss == 'Alta')
                                            {{-- 🔥 NUEVO BOTÓN PARA EDITAR SDI/PATRÓN/FECHA SI YA ESTÁ DE ALTA --}}
                                            <button type="button" class="btn btn-sm btn-warning btn-registrar-alta-imss text-dark"
                                                data-bs-toggle="modal" data-bs-target="#modalAltaImss"
                                                data-id_empleado="{{ $empleado->id_empleado }}"
                                                data-nombre_empleado="{{ $empleado->nombre_completo }}"
                                                data-fecha_alta_actual="{{ $empleado->fecha_alta_imss ? $empleado->fecha_alta_imss->toDateString() : '' }}"
                                                data-id_patron_imss_actual="{{ $empleado->id_patron_imss }}"
                                                data-sdi_actual="{{ $empleado->sdi ?? '' }}"
                                                title="Actualizar Datos IMSS (SDI, Patrón, etc.)">
                                                <i class="bi bi-pencil-square"></i>
                                            </button>

                                            {{-- Documentos PDF --}}
                                            @if ($empleado->fecha_alta_imss && $empleado->id_patron_imss)
                                            ...
                                            @else
                                                <button type="button" class="btn btn-sm btn-outline-primary" disabled title="Falta información para generar PDF">
                                                    <i class="bi bi-file-earmark-pdf"></i> 
                                                </button>
                                            @endif
                                            ...
                                            <button type="button" class="btn btn-sm btn-danger btn-registrar-baja-imss"
                                                ...
                                                data-id_empleado="{{ $empleado->id_empleado }}"
                                                data-nombre_empleado="{{ $empleado->nombre_completo }}"
                                                data-fecha_alta_imss="{{ $empleado->fecha_alta_imss ? $empleado->fecha_alta_imss->toDateString() : '' }}"
    ...
    <div class="modal fade" id="modalAltaImss" tabindex="-1" aria-labelledby="modalAltaImssLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="formAltaImss" method="POST" action="">
                    @csrf
            ...
            <div class="modal-content">
                ...
                <form id="formBajaImss" method="POST" action="">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="hidden" name="id_empleado_baja_imss" id="id_empleado_baja_imss_modal"> 
